...
"""
Usage:
    pytest test_min_perf.py --level core --warmup 1000 --iter 1000 \
...
class MINBenchmark(Benchmark):
    ...
    def get_input_iter(self, cur_dtype) -> Generator:
        for shape in self.shapes:
            ...
            # Please note that `torch.min` will not accept the `dim` param to be None.
            if form_detail_dim is not None:
                ...
                # param to be None in the `process_min` script.
                yield {
                    "input": inp,
                },
...
def min_input_fn(shape, cur_dtype, device):
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "min",
            torch.min,
            marks=pytest.mark.min,
        ),
    ],
)
def test_min_benchmark(op_name, torch_op):
    bench = MINBenchmark(
        # input_fn=input_fn,
        op_name=op_name,
        torch_op=torch_op,
