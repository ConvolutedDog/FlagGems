...
"""
Usage:
    pytest test_argmin_perf.py --level core --warmup 1000 --iter 1000 \
...
def argmin_input_fn(shape, cur_dtype, device):
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "argmin",
            torch.argmin,
            marks=pytest.mark.argmin,
        ),
    ],
)
def test_argmin_benchmark(op_name, torch_op):
    bench = ARGMINBenchmark(
        # input_fn=input_fn,
        op_name=op_name,
        torch_op=torch_op,
        dtypes=FLOAT_DTYPES,
