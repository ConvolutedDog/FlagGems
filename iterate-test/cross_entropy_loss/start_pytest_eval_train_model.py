...
pytest_operation_name = "cross_entropy_loss"
...
excel_config = {
    ...
    "dtype_col": "dtype",
    # Shape parameters.
    "shape_cols": ["shape_detail_N", "shape_detail_L"],
    ...
    # Auto-tune configs.
    "config_cols": ["BLOCK_C", "BLOCK_D", "num_warps"],
    # Performance.
    "latency_col": "latency",
    # Benchmark name of shape yaml.
    "bench_name": "CROSSENTROPYLOSSBenchmark",
...
def gen_form_detail_has_weight():
    return [1, 0]


# 0 | 1 | 2 for 'none' | 'mean' | 'sum'.
def gen_form_detail_reduction():
    return [0, 1, 2]


def gen_form_detail_label_smoothing():
    return [0.0, 0.5]
...
def gen_BLOCK_C(
    ...
    """
    res = 5.02754660e-5 * shape_detail_L + 5.70746029e-05 * shape_detail_N + 0.951953125
...
shapegen = ShapeGenerator(
    excel_config,
    (gen_shape_detail_N, gen_shape_detail_L),
    form_generators=(
        gen_form_detail_has_weight,
        gen_form_detail_reduction,
        gen_form_detail_label_smoothing,
    ),
)
...
tunedconfiggen = TunedConfigGenerator(
    excel_config,
    (gen_BLOCK_C, gen_BLOCK_D, gen_num_warps),
    shapegen,
)
