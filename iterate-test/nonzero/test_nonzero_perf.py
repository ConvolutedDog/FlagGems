...
from performance_utils import GenericBenchmark2DOnly, unary_input_fn
...
"""
Usage:
    pytest test_nonzero_perf.py --level core --warmup 1000 --iter 1000 \
...
class NONZEROBenchmark(GenericBenchmark2DOnly):
    """Benchmark for nonzero."""
...
def nonzero_input_fn(shape, cur_dtype, device):
    return unary_input_fn(shape, cur_dtype, device)
...
def test_nonzero_benchmark(op_name, torch_op, input_fn):
    bench = NONZEROBenchmark(
        input_fn=input_fn,
        op_name=op_name,
        torch_op=torch_op,
        dtypes=FLOAT_DTYPES,
