...
from performance_utils import GenericBenchmarkExcluse1D
...
"""
Usage:
    pytest test_triu_perf.py --level core --warmup 1000 --iter 1000 \
...
class TRIUBenchmark(GenericBenchmarkExcluse1D):
    """Benchmark for triu."""
...
def triu_input_fn(shape, dtype, device):
    batch, channel, height, weight, form_detail_diagonal = shape
    shape = batch, channel, height, weight
    input = torch.randn(size=shape, device=device, dtype=dtype)
    yield {
        "input": input,
        "diagonal": form_detail_diagonal,
    },
