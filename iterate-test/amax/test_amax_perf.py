...
"""
Usage:
    pytest test_amax_perf.py --level core --warmup 1000 --iter 1000 \
...
def amax_input_fn(shape, cur_dtype, device):
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "amax",
            torch.amax,
            marks=pytest.mark.amax,
        ),
    ],
)
def test_amax_benchmark(op_name, torch_op):
    bench = AMAXBenchmark(
        # input_fn=input_fn,
        op_name=op_name,
        torch_op=torch_op,
