...
"""
Usage:
    pytest test_var_mean_perf.py --level core --warmup 1000 --iter 1000 \
...
class VARMEANBenchmark(Benchmark):
    """Benchmark for var_mean."""
    ...
    def get_input_iter(self, cur_dtype) -> Generator:
        for shape in self.shapes:
            ...
            # Please note that `torch.var_mean` will not accept the `dim` param to be None.
            if form_detail_dim is not None:
                yield {
                    "input": inp,
                    "dim": form_detail_dim,
                    "keepdim": False,
                ...
                # For this situation, we must remember that we need to record the `dim`
                # param to be None in the `process_var_mean` script.
                yield {
                    "input": inp,
                },
...
def var_mean_input_fn(shape, cur_dtype, device):
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "var_mean",
            torch.var_mean,
            marks=pytest.mark.var_mean,
        ),
    ],
)
def test_var_mean_benchmark(op_name, torch_op):
    bench = VARMEANBenchmark(
        # input_fn=input_fn,
        op_name=op_name,
        torch_op=torch_op,
        dtypes=FLOAT_DTYPES,
