...
"""
Usage:
    pytest test_sum_perf.py --level core --warmup 1000 --iter 1000 \
...
class SUMBenchmark(Benchmark):
    ...
    def get_input_iter(self, cur_dtype) -> Generator:
        for shape in self.shapes:
            ...
            # Please note that `torch.sum` will not accept the `dim` param to be None.
            if form_detail_dim is not None:
                ...
                # param to be None in the `process_sum` script.
                yield {
                    "input": inp,
                },
...
def sum_input_fn(shape, cur_dtype, device):
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "sum",
            torch.sum,
            marks=pytest.mark.sum,
        ),
    ],
)
def test_sum_benchmark(op_name, torch_op):
    bench = SUMBenchmark(
        # input_fn=input_fn,
        op_name=op_name,
        torch_op=torch_op,
