...
"""
Usage:
    pytest test_mv_perf.py --level core --warmup 1000 --iter 1000 \
...
class MVBenchmark(Benchmark):
    ...
    def get_tflops(self, op, *args, **kwargs):
        ...
        # shape(m,k)(k,n)
        # total_flops mxnx2k
        assert self.op_name == "mv", "The operation is not mv!"
        total_flops = args[0].shape[0] * args[0].shape[1] * 2
...
def test_mv_benchmark(op_name, torch_op, input_fn):
    bench = MVBenchmark(
        input_fn=input_fn,
        op_name=op_name,
        torch_op=torch_op,
