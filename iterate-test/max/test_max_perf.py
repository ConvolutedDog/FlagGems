...
"""
Usage:
    pytest test_max_perf.py --level core --warmup 1000 --iter 1000 \
...
class MAXBenchmark(Benchmark):
    ...
    def get_input_iter(self, cur_dtype) -> Generator:
        for shape in self.shapes:
            ...
            # Please note that `torch.max` will not accept the `dim` param to be None.
            if form_detail_dim is not None:
                ...
                # param to be None in the `process_max` script.
                yield {
                    "input": inp,
                },
...
def max_input_fn(shape, cur_dtype, device):
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "max",
            torch.max,
            marks=pytest.mark.max,
        ),
    ],
)
def test_max_benchmark(op_name, torch_op):
    bench = MAXBenchmark(
        # input_fn=input_fn,
        op_name=op_name,
        torch_op=torch_op,
