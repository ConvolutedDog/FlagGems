        ...
        if filename.endswith(".cfg"):
            ...
            with open(cfg_file_path, "r") as file:
                ...
                for line in file:
                    ...
                    output_h = int(parts[3])  # Ho
                    output_w = int(parts[4])  # Wo
                    out_c = int(parts[6])  # D

                    for batch in [1, 4, 8, 16, 32]:
                        # BUG: Ther is a bug if we provide param `dim=0` for flaggems.
                        for dim in [None, 1, 2, 3]:
                            shapes.append(
                                {
                                    "shape_detail_N": batch,
                                    "shape_detail_C": out_c,
                                    "shape_detail_H": output_h,
                                    "shape_detail_W": output_w,
                                    "form_detail_dim": dim,
                                }
                            )
        ...
        if filename.endswith(".cfg"):
            ...
            with open(cfg_file_path, "r") as file:
                ...
                for line in file:
                    ...
                    params = (
                        int(parts[6]),  # D (out_c)
                        int(parts[3]),  # Ho (output_h)
                        int(parts[4]),  # Wo (output_w)
                    )
                    global_cfg_params.add(params)


def check_params_in_global(out_c, output_h, output_w):
    """
...
def constraint_real_conv_params(**kwargs):
    ...
    out_c = kwargs["shape_detail_C"]
    output_h = kwargs["shape_detail_H"]
    output_w = kwargs["shape_detail_W"]

    # Check if the parameters exist in the global variable
    return check_params_in_global(out_c, output_h, output_w)
...
configgen = ConfigGenerator(
    excel_config,
    (gen_BLOCK_M, gen_BLOCK_N, gen_num_warps),
)
