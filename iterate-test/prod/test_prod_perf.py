...
"""
Usage:
    pytest test_prod_perf.py --level core --warmup 1000 --iter 1000 \
...
class PRODBenchmark(Benchmark):
    ...
    def get_input_iter(self, cur_dtype) -> Generator:
        for shape in self.shapes:
            ...
            # Please note that `torch.prod` will not accept the `dim` param to be None.
            if form_detail_dim is not None:
                ...
                # param to be None in the `process_prod` script.
                yield {
                    "input": inp,
                },
...
def prod_input_fn(shape, cur_dtype, device):
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "prod",
            torch.prod,
            marks=pytest.mark.prod,
        ),
    ],
)
def test_prod_benchmark(op_name, torch_op):
    bench = PRODBenchmark(
        # input_fn=input_fn,
        op_name=op_name,
        torch_op=torch_op,
